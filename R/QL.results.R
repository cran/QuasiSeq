...
QL.results<-function(fit,Dispersion="Deviance",spline.df=NULL,Plot=FALSE){
...
if(Plot){
	...
	plot(log(mn.cnt),y,xlab=expression(log(bar(y)[phantom()%.%phantom()]*phantom()[phantom()%.%phantom()])),
...
phi.spline<-(D0*exp(spline.fit$fitted.values)*phi0+(den.df)*phi.hat)/(D0+den.df-2)
...
pval[[i]]<-cbind(1-pf(LRT[,i]/phi.hat2,num.df[i],den.df),1-pf(LRT[,i]/phi.shrink,num.df[i],est.d0+den.df),1-pf(LRT[,i]/phi.spline,num.df[i],D0+den.df))
colnames(pval[[i]])<-c(paste(Model,"QL"),paste(Model," QLShrink d0=",round(est.d0,1),sep=""),
paste(Model," QLSpline d0=",round(D0,1),sep="")) 
}
...
return(list(P.values=pval,Q.values=qval,m0=m0))
}
